...
// Opcional: Mapa para buscar rápidamente el `word` por nombre del personaje
export const CharacterWords: { [key: string]: string } = Characters.reduce(
  (acc, char) => ({
    ...acc,
    [char.name]: char.word || `${char.name}です`,
  }),
  {}
);
