...
void main() {
    ...
    // Calculate distance from center of disc (UV space)
    vec2 uvCenter = vec2(0.5, 0.5);
    float dist = length(vUvs - uvCenter);
    float borderWidth = 0.004; // Adjust this for thicker/thinner border
    float borderRadius = 0.5 - borderWidth; // Inner radius of the border

    // Check if fragment is in the border region
    if (dist > borderRadius && dist <= 0.5) {
        outColor = vec4(1.0, 1.0, 1.0, vAlpha); // White border with same alpha
    } else if (dist <= borderRadius) {
        outColor = texture(uTex, st);
        outColor.a *= vAlpha;
    } else {
        discard; // Discard fragments outside the disc
    }
}
`;
...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, onActiveItemPositionChange = () => {}, onItemLog = () => {}) {
    this.canvas = canvas;
    this.items = items || [];
    this.onActiveItemChange = onActiveItemChange || (() => { });
    this.onMovementChange = onMovementChange || (() => { });
    this.onActiveItemPositionChange = onActiveItemPositionChange || (() => {});
    this.onItemLog = onItemLog || (() => {});
    this.#init(onInit);
  }
  ...
  #onControlUpdate(deltaTime) {
    ...
    if (isMoving !== this.movementActive) {
      ...
      if (!isMoving) {
        // Movement stopped, find and log the active item
        const nearestVertexIndex = this.#findNearestVertexIndex();
        const itemIndex = nearestVertexIndex % Math.max(1, this.items.length);
        this.onActiveItemChange(itemIndex);
        this.onItemLog(this.items[itemIndex]); // Log item details

        const vertexPos = this.instancePositions[nearestVertexIndex];
        const screenPos = this.#projectToScreen(vertexPos);
        this.onActiveItemPositionChange(screenPos);
      }
    }
    ...
    if (!this.control.isPointerDown) {
      ...
      // Calculate screen position of the active item
      const vertexPos = this.instancePositions[nearestVertexIndex];
      const screenPos = this.#projectToScreen(vertexPos);
      this.onActiveItemPositionChange(screenPos);
  ...
  #projectToScreen(position) {
    const gl = this.gl;
    const worldPos = vec3.transformQuat(vec3.create(), position, this.control.orientation);
    const viewPos = vec4.transformMat4(vec4.create(), [...worldPos, 1.0], this.camera.matrices.view);
    ...
    const screenX = ((ndc[0] + 1.0) * canvasWidth) / 2.0;
    const screenY = ((1.0 - ndc[1]) * canvasHeight) / 2.0; // Flip Y-axis
...
export default function InfiniteMenu({ items = [] }) {
  ...
  const [activeItemPosition, setActiveItemPosition] = useState({ x: 0, y: 0 });
  ...
  useEffect(() => {
    ...
    const handleActiveItemPosition = (position) => {
      ...
      const handleMovementChange = (isMoving) => {
        ...
        if (isMoving) {
          setHasLoggedItem(false); // Reset when movement starts
        }
      };
  
      const handleItemLog = (item) => {
        if (!hasLoggedItem) {
            handleSelectedItem(item);
            setHasLoggedItem(true); // Prevent further logs until movement
        }
      };
  ...
  useEffect(() => {
    //firstTimeLog
    if (activeItem && !hasLoggedItem && !hasLoggedItemFirstTime) {
        handleSelectedItem(activeItem);
        setHasLoggedItemFirstTime(true);
    }
  }, [activeItem, hasLoggedItem]);

  const handleButtonClick = () => {
    if (!activeItem?.link) return;

  // Use the current domain as the base URL, or specify a custom one
  const baseUrl = window.location.origin; // e.g., https://yourdomain.com
  // Alternatively, hardcode if needed: const baseUrl = 'https://yourdomain.com';

  // Ensure the link starts with '/' and prepend base URL
  const fullUrl = activeItem.link.startsWith('/')
    ? `${baseUrl}${activeItem.link}`
    : activeItem.link;
  ...
  window.location.href = fullUrl;
   ...
   // Calculate percentage offsets (responsive)
   const isMobile = canvasSize.width < 640; // Adjust threshold as needed
   const isMediumScreen = canvasSize.width < 1400;
   const baseOffsetX = isMobile ? 4 : isMediumScreen ? 28 : 18; // ~16.67% to 27.86% for left/right
   const baseOffsetY = isMobile ? 25 : 23; // ~23.15% for top/bottom
   const minOffsetPx = isMobile ? -10 : 0; // Minimum offset to stay near disc
 
   // Convert item position to percentages
   const itemXPercent = (activeItemPosition.x / canvasSize.width) * 100;
   const itemYPercent = (activeItemPosition.y / canvasSize.height) * 100;


  return (
    <div className="fixed w-screen h-screen top-0 left-0 m-0 p-0 overflow-hidden">
      <canvas
      ...
      {activeItem && (
        <>
          ...
          style={{
              position: 'absolute',
              left: `calc(${itemXPercent}% - ${baseOffsetX}% - ${minOffsetPx}px - 150px)`,
              top: `calc(${itemYPercent}% - ${baseOffsetY}% - ${minOffsetPx}px)`,
              zIndex: 10,
              fontSize: '40px',
              fontWeight: '600',
              display: 'flex',
        ...
        `}
          >
            {activeItem.title} <span  className="mt-4"><SquareArrowOutUpRight size={24} /></span> 
          </h2>
          ...
          style={{
              position: 'absolute',
              left: `calc(${itemXPercent}% + ${baseOffsetX}% + ${minOffsetPx}px)`,
              top: `calc(${itemYPercent}% + ${baseOffsetY}% + ${minOffsetPx}px)`,
              zIndex: 10,
              fontSize: '24px',
            }}
